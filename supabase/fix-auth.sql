-- Run this in Supabase → SQL Editor if admin login fails
-- Safe to re-run
...
-- Ensure default admin credentials exist / are reset to the known defaults
insert into public.admin_users (username, password_hash)
values ('superspeeddelivery', extensions.crypt('superspeeddeliverypass', extensions.gen_salt('bf')))
...
begin
  ...
  if p_id is null then
    ...
    update public.blogs
    set title = trim(p_title), content = trim(p_content), images = v_images,
